...
const Sidebar = ({ 
  ...
  const menuItems = [
    { name: "Dashboard", icon: <RiDashboardLine size={22} />, path: "/admin/dashboard" },
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-20 bg-black/50 transition-all duration-300 lg:hidden ${
          isMobileOpen 
      ...
      {isCollapsed && (
        <button
          ...
          className="fixed top-6 left-5 z-40 hidden lg:flex items-center justify-center w-10 h-10 bg-white border border-slate-200 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-200"
          aria-label="Expand sidebar"
        >
          <RiArrowRightSLine 
            size={20} 
            className={`text-slate-600 transition-transform ${isHovered ? "translate-x-0.5" : ""}`} 
          />
        </button>
      )}
      ...
      <aside
        className={`fixed top-0 left-0 z-30 h-screen bg-white border-r border-slate-200 transition-all duration-300 ease-in-out flex flex-col
      ...
      >
        ...
        <div className={`flex items-center h-16 border-b border-slate-100 transition-all ${isCollapsed ? "justify-center px-0" : "justify-between px-6"}`}>
          ...
          <Link 
            ...
            onClick={() => setIsMobileOpen(false)}
          >
            <div className="min-w-[32px] h-8 rounded-lg bg-gradient-to-br from-rose-600 to-pink-600 flex items-center justify-center text-white font-bold font-serif text-lg shadow-sm">
              S
            </div>
          ...
          <button 
            onClick={() => setIsMobileOpen(false)} 
            className="lg:hidden text-slate-500 hover:text-rose-600 hover:bg-rose-50 p-1.5 rounded-lg transition-colors"
            ...
            return (
              ...
              >
                <Link
                  ...
                  `}
                >
                  <span className={`transition-colors min-w-[24px] flex justify-center ${isActive ? "text-rose-600" : "text-slate-400 group-hover:text-slate-600"}`}>
                    {item.icon}
                  </span>
                  ...
                  {isActive && isCollapsed && (
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-rose-500"></div>
                  )}
                </Link>
        ...
        <div className="border-t border-slate-100 bg-white/95 backdrop-blur-sm">
          ...
          <div className="p-3">
            <button 
              ...
              className="hidden lg:flex items-center justify-between w-full py-2.5 px-3 text-slate-600 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all duration-200 group"
          ...
          <div className="p-3 pt-0">
            <button className={`flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-slate-600 rounded-xl hover:bg-red-50 hover:text-red-600 transition-all duration-200 group ${isCollapsed ? "justify-center px-2" : ""}`}>
